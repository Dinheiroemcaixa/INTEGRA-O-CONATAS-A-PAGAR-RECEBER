export const CATEGORIAS_DRE_CONTA_AZUL = {
  '4.01 Impostos sobre Vendas e sobre Serviços': [
    'DIFAL',
    'Estornos e Cancelamentos',
    'ICMS',
    'ICMS ST sobre Vendas',
    'IRPJ/CSLL',
    'ISS sobre Faturamento',
    'PIS/COFINS',
    'Simples Nacional - DAS'
  ],
  '4.02 Despesas com Vendas e Serviços': [
    'Comissões de Vendedores',
    'Franquia e Royalties',
    'Frete',
    'Insumos/Materials de Oficina',
    'Materiais Aplicados na Prestação de Serviços',
    'Materiais para Revenda',
    'Transporte de Mercadorias Vendidas'
  ],
  '4.03 Despesas com Salários e Encargos': [
    '13º Salário - 1ª Parcela',
    '13º Salário - 2ª Parcela',
    'Adiantamento Salarial',
    'Férias',
    'FGTS e Multa de FGTS',
    'Gratificações',
    'INSS sobre Salários - GPS',
    'IRRF s/ Salários - DARF 0561',
    'PLR - Participação nos Lucros e Resultados',
    'Remuneração de Autônomos',
    'Remuneração de Estagiários',
    'Rescisões',
    'Salários',
    'Vale-Alimentação',
    'Vale-Transporte'
  ],
  '4.04 Despesas com Colaboradores': [
    'Confraternizações',
    'Contribuição Sindical',
    'Cursos e Treinamentos',
    'EPI\'s',
    'Exames Médicos',
    'Farmácia',
    'Plano de Saúde Colaboradores',
    'Plano Odontológico Colaboradores',
    'Relógio de Ponto',
    'Seguro de Vida',
    'Uniformes'
  ],
  '4.05 Despesas Administrativas': [
    'Bens de Pequeno Valor',
    'Cartório',
    'Copa e Cozinha',
    'Correios',
    'Honorários Advocatícios',
    'Honorários Consultoria',
    'Honorários Contábeis',
    'Honorários (outros)',
    'Indenizações e Acordos Judiciais',
    'Lanches e Refeições',
    'Manutenção de Equipamentos',
    'Manutenção de Sistemas',
    'Materiais de Escritório',
    'Materiais de Limpeza e de Higiene',
    'Retenção - Darf 1708 - IRRF',
    'Retenção - Darf 5952 - PIS/COFINS/CSLL',
    'Retenção - GPS 2631 - INSS',
    'Retenção - ISS Serviços Tomados',
    'Serviços de Limpeza',
    'Telefonia e Internet',
    'Telefonia Móvel',
    'Transporte Urbano (táxi, Uber)'
  ],
  '4.06 Despesas Comerciais': [
    'Brindes para Clientes',
    'Marketing e Publicidade',
    'Viagens e Representações'
  ],
  '4.07 Despesas com Imóvel': [
    'Água e Saneamento',
    'Aluguel',
    'Alvará de Funcionamento',
    'Condomínio',
    'Energia Elétrica',
    'IPTU',
    'Manutenção Predial',
    'Retenção - Darf 3208 - IRRF Aluguel',
    'Seguro de Imóveis',
    'Taxa de Lixo',
    'Taxas Municipais',
    'Vigilância e Segurança Patrimonial'
  ],
  '4.08 Despesas com Veículos': [
    'Combustíveis',
    'Estacionamento',
    'IPVA / DPVAT / Licenciamento',
    'Manutenção de Veículos',
    'Multas de Trânsito',
    'Pedágios',
    'Seguros de Veículos'
  ],
  '4.09 Despesas com Diretoria': [
    'Antecipação de Lucros',
    'Despesas Pessoais dos Sócios',
    'INSS sobre Pró-labore - GPS',
    'IRRF sobre Pró-labore - Darf',
    'Plano de Saúde Sócios',
    'Plano Odontológico Sócios',
    'Pró-labore'
  ],
  '4.10 Despesas Financeiras': [
    'Impostos sobre Aplicações',
    'Tarifas Bancárias',
    'Tarifas de Antecipações de Cartões',
    'Tarifas de Boletos',
    'Tarifas de Cartões de Crédito',
    'Tarifas DOC / TED'
  ],
  '4.11 Outras Despesas': [
    'Despesas a identificar'
  ],
  '5.01 Bens Imobilizados da Empresa': [
    'Benfeitorias em Bens de Terceiros',
    'Computadores e Periféricos',
    'Construções em Andamento - Imóvel Próprio',
    'Edifícios e Construções',
    'Leasing - Imóveis',
    'Leasing - Máquinas, Equipamentos e Instalações Industriais',
    'Leasing - Móveis, Utensílios e Instalações Administrativos',
    'Leasing - Móveis, Utensílios e Instalações Comerciais',
    'Leasing - Outras Imobilizações',
    'Leasing - Veículos',
    'Máquinas, Equipamentos e Instalações Industriais',
    'Móveis, Utensílios e Instalações Administrativos',
    'Móveis, Utensílios e Instalações Comerciais',
    'Outras Imobilizações por Aquisição',
    'Software / Licença de Uso',
    'Terrenos',
    'Veículos'
  ],
  '5.02 Empréstimos e Financiamentos': [
    'Empréstimos de Bancos',
    'Empréstimos de Outras Instituições',
    'Empréstimos de Sócios',
    'Juros Conta Garantida',
    'Pagamento de Empréstimo de Mútuo'
  ],
  '5.03 Parcelamentos e Dívidas': [
    'Parcelamento do Simples Nacional',
    'Descontos financeiros concedidos',
    'Descontos incondicionais concedidos',
    'Fretes pagos',
    'Impostos retidos em vendas',
    'Juros pagos',
    'Multas pagas',
    'Perdas',
    'Sem Categoria',
    'Tarifas'
  ]
} as const;

export const CONTAS_FINANCEIRAS_PADRAO = [
  'Aplicação Automática Santander Barão',
  'Caixa Dinheiro',
  'Hanover Itaú',
  'Itaú Nufast Barão',
  'Phenix Itaú',
  'Santander Barão',
  'Tesouraria (Aporte/Sangria)',
  'Vendas Cartões'
] as const;

export const LISTA_CATEGORIAS_FLAT = Object.values(CATEGORIAS_DRE_CONTA_AZUL).flat();
