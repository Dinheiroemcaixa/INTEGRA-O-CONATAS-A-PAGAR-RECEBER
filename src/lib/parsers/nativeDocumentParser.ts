...
export function extrairDadosDeTextoNativo(texto: string): DadosDocumentoExtraido {
  ...
  if (textoUpper.includes('FGTS DIGITAL') || textoUpper.includes('GFD - GUIA') || textoUpper.includes('FUNDO DE GARANTIA DO TEMPO DE SERVICO') || textoUpper.includes('GRRF')) {
    ...
    const matchTag = texto.match(/Tag\s*[\:\-]?\s*(?:GRRF\s*[\:\-]?\s*)?([A-Z\s]{3,50})/i) ||
                     texto.match(/GRRF\s*[\:\-]?\s*([A-Z\s]{3,50})/i)
    ...
    if (ehRescisorio) {
      ...
      if (nomeColaborador) {
        ...
        // Tenta buscar razão social do empregador se não achou nome do colaborador
    ...
    // e) PIX Copia e Cola / Payload Location
